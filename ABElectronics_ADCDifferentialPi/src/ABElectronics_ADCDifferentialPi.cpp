...
char ABElectronics_ADCDifferentialPi::updateByte(char byte, char bit, char value) {
  /*
...
ABElectronics_ADCDifferentialPi::ABElectronics_ADCDifferentialPi(byte address, byte address2)
{
...
float ABElectronics_ADCDifferentialPi::readVoltage(int channel) {
  // returns the voltage from the selected adc channel - channels 1 to 8
  long raw = readRaw(channel);
  if (signBit == 1) {
       return float(raw * (lsb / pga) - 2.048) ;
     } else{
      return float(raw * (lsb / pga));
   }
}

long ABElectronics_ADCDifferentialPi::readRaw(int channel) {
...
void ABElectronics_ADCDifferentialPi::setChannel(int channel) {
...
void ABElectronics_ADCDifferentialPi::setPGA(int gain) {
  // PGA gain selection
  //1 = 1x
  // 2 = 2x
  // 4 = 4x
...
void ABElectronics_ADCDifferentialPi::setBitRate(int rate) {
  //
...
void ABElectronics_ADCDifferentialPi::setConversionMode(int mode) {
...
void ABElectronics_ADCDifferentialPi::writeI2C(byte address, byte msg)
{
